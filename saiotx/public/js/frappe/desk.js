...
frappe.Application = Class.extend({
	...
	setup_profile_template(){
		...
		let sales_order_template = "";
		let purchase_order_template = "";
		let you_have_text = __("You have");
		let check_in_text = __("Check In");
		let check_out_text = __("Check Out");

		if (frappe.boot.lang === "ar"){
			you_have_text = "لديك";
			check_in_text = "تسجيل حضور";
			check_out_text = "تسجيل انصراف"
		}
		if(frappe.user.has_role("Sales User") || frappe.user.has_role("Sales Manager")){
			...
			<div class="skills sales-invoice">
				...
				<ul>
					<a href="#List/Sales Invoice"><li>${you_have_text} <span id="sales-invoices-available">${sales_invoice_counter}</span> ${__("Sales Invoice")}. </li></a>
				</ul>
			</div>
			`;
		}
		...
		if(frappe.user.has_role("Sales User") || frappe.user.has_role("Sales Manager")){
			//TODO get sales invoice counter
			let sales_order_counter = 0;
			sales_order_template = `
			<div class="skills sales-order">
				<h6>${__("Sales Invoice")}</h6>
				<ul>
					<a href="#List/Sales Order"><li>${you_have_text} <span id="sales-orders-available">${sales_order_counter}</span> ${__("Sales Order")}. </li></a>
				</ul>
			</div>
			`;
		}
		...
		if(frappe.user.has_role("Purchase User") || frappe.user.has_role("Purchase Manager")){
			...
			<div class="skills purchase-invoice">
				...
				<ul>
				<a href="#List/Purchase Invoice"><li>${you_have_text} <span id="purchase-invoices-available">${purchase_invoice_counter}</span> ${__("Purchase Invoice")}. </li></a>
				</ul>
			</div>
			`;
		}
		...
		if(frappe.user.has_role("Purchase User") || frappe.user.has_role("Purchase Manager")){
			//TODO get sales invoice counter
			let purchase_order_counter = 0;
			purchase_order_template = `
			<div class="skills purchase-order">
				<h6>${__("Purchase Order")}</h6>
				<ul>
				<a href="#List/Purchase Order"><li>${you_have_text} <span id="purchase-orders-available">${purchase_order_counter}</span> ${__("Purchase Order")}. </li></a>
				</ul>
			</div>
			`;
		}
			...
			<div class="card-container">
				...
				<div class="buttons">
					<button class="primary">
						<a href="${user_profile_path}">${__("My Profile")}</a>
					</button>
					<button class="primary ghost">
						<a href="${user_settings_path}">${__("Settings")}</a>
					</button>
				</div>
				${purchase_invoice_template}
				${sales_invoice_template}
				...
				<div class="skills">
					...
					<ul>
						...
						<a href="#List/Employee Checkin"><li>${check_in_text}</li></a>
						<li id="check-in-field">${__("-")}</li>
						<br/>
						<li style="visibility: hidden;">${frappe.datetime.get_today()}</li>
						<a href="#List/Employee Checkin"><li>${check_out_text}</li></a>
						<li id="check-out-field">${__("-")}</li>
					</ul>
				</div>
			</div>
		...
		if(frappe.user.has_role("Sales User") || frappe.user.has_role("Sales Manager")){
			frappe.db.count("Sales Order").then(count=>{$('#sales-orders-available')[0].innerHTML = count})
		}

		if(frappe.user.has_role("Purchase User") || frappe.user.has_role("Purchase Manager")){
			frappe.db.count("Purchase Order").then(count=>{$('#purchase-orders-available')[0].innerHTML = count})
		}
				...
				if(result.message){
					...
					for (let i = 0; i < info.length; i++) {
						...
						if(check_info.log_type == "IN"){
							$('#check-in-field')[0].innerHTML = `<a href="#Form/Employee Checkin/${check_info.name}"}>` + check_info.time.split(" ")[1]+"</a>";
						}
						if(check_info.log_type == "OUT"){
							$('#check-out-field')[0].innerHTML = `<a href="#Form/Employee Checkin/${check_info.name}"}>` + check_info.time.split(" ")[1]+"</a>";
						}
					}
				}
			}
